...
test('pick one if onStart has more than one entry', () => {
	...
	for (let i = 0; i < 100; i++) subject.onRunStart(ar(), { estimatedTime: 10.01, showStatus: false })
...
test('pick one if onSuitePass is an array', () => {
	...
	for (let i = 0; i < 100; i++) subject.onRunComplete({}, ar({ numTotalTestSuites: 1, numFailedTestSuites: 0 }))
...
test('pick one if onSuiteFailure is an array', () => {
	...
	for (let i = 0; i < 100; i++) subject.onRunComplete({}, ar({ numTotalTestSuites: 1, numFailedTestSuites: 1 }))
